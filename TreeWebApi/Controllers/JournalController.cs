...
public class JournalController : BaseMediatorController
{
    ...
    public async Task<IActionResult> GetRange(int skip, int take, [FromBody] Filter filter, CancellationToken cancellationToken)
    {
        var logs = await _mediator.Send(
            new GetRangeQuery(new GetRangeContext(skip, take, filter)), cancellationToken
            );
